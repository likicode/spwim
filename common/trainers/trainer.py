class Trainer(object):
    ...
    def __init__(self, model, embedding, train_loader, trainer_config, train_evaluator, test_evaluator, dev_evaluator=None):
        ...
        self.logOutput = trainer_config.get('logOutput')   
    ...
    def evaluate(self, evaluator, dataset_name, epoch, logf):
        ...
        if self.logger is not None:
            ...
            self.logger.info('\t'.join([dataset_name] + list(map(str, scores))))
            # write output tofile
            f.write('Evaluation metrics for {}'.format(dataset_name))
            f.write("\n")
            f.write('\t'.join([' '] + metric_names))
            f.write("\n")
            f.write('\t'.join([dataset_name] + list(map(str, scores))))
            f.write("\n")
        return scores

    def evaluate_sts_test(self, evaluator, dataset_name, epoch, logf):
        scores, metric_names = evaluator.get_sts_test_scores()
        f = open(logf, "a+")
        f.write("Epoch: {}\n".format(epoch))
        if self.logger is not None:
            self.logger.info('Evaluation metrics for {}:'.format(dataset_name))
