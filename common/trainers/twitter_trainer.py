...
class TwitterTrainer(Trainer):
    def train_epoch(self, epoch):
        ...
        if self.use_tensorboard:
            self.writer.add_scalar('twitter/train/nll_loss', total_loss / len(self.train_loader.dataset.examples), epoch)
    ...
    def train(self, epochs):
        ...
        for epoch in range(1, epochs + 1):
            ...
            f1, new_loss = self.evaluate(self.dev_evaluator, 'dev', epoch, self.logOutput)

            if self.use_tensorboard:
                self.writer.add_scalar('twitter/lr', self.optimizer.param_groups[0]['lr'], epoch)
                self.writer.add_scalar('twitter/dev/f1', f1, epoch)
                self.writer.add_scalar('twitter/dev/kl_div_loss', new_loss, epoch)
